...
def run(selected_team: str) -> None:
    ...
    kpi_data = [
        ...
        },
        {
            ...
            'detailed_description': 'This metric measures the load-bearing capacity of the structure per unit area.'
        }
    ]
    ...
    # Call the display function for Structure Metrics
    display_metric_circles_and_tape(
        st.container(),
        title="Column-Free Floor Area Ratio",
        primary_metric_value=column_free_floor_area_metric.calculate(),  # Use the calculated structure metric value
        metric_values=[total_column_free_floor_area, total_floor_area],  # Use only the column-free floor area metric value
        metric_names=["Total Column-Free Floor Area", "Total Floor Area"],  # Use only the column-free floor area metric name
    ...
    display_metric_circles_and_tape(
        st.container(),
        title="Load Capacity per Square Meter",
        primary_metric_value=load_capacity_metric.calculate(),  # Use the calculated structure metric value
        metric_values=[load_capacity, self_weight_of_structure],  # Use only the load capacity metric value
        metric_names=["Load Capacity", "Self Weight of Structure"],  # Use only the load capacity metric name
        units="kg/m²"  # Adjust units as necessary
    )
